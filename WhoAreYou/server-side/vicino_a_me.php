<?php
   ...
   ***** VICINO A ME *****
   ...
   -> per ogni coordinate vicine stampo le informazioni delle persone presenti in tale posizione
   ...
   */
   ...
   for ($i=0; $i < count($arrayDistanza); $i++) {
      ...
      $query3 = "SELECT A.identificativo, A.immagine, A.username, RP.data_ora FROM Registrazione_Posizione RP JOIN Account A ON RP.Account_identificativo=A.identificativo WHERE Coord_latitudine='".$latDistanza."' AND Coord_longitudine='".$longDistanza."' AND data_ora >='".$dateAnHourAgo."' ORDER BY RP.data_ora DESC ";

      if( $risultato3 = mysqli_query($link, $query3, MYSQLI_USE_RESULT) ) {

         while ($row = mysqli_fetch_array($risultato3)) {

            if( $row['identificativo'] != $identificativo ) {

               $arrayAccountVicinoAMe = array('identificativo' => $row['identificativo'], 'immagine' => $row['immagine'], 'username' => $row['username'], 'data_ora' => $row['data_ora'], 'distanza' => $distanza);

               array_push($arrayVicinoAMe, $arrayAccountVicinoAMe);
            }
   ...
   $status = "OK";
   $jsonArr = array('status' => $status, 'result' => $arrayVicinoAMe);
